...
int main(int argc, char *argv[]) {
  if (argc < 2) {
    fprintf(stderr, "Usage: ./random-write-mpi-multiple entries <mode> "
  ...
  // Peek at the mode: for aggregator-less writing, we don't need
  // MPI_THREAD_MULTIPLE.
  if ((mode & 8) != 0) {
  ...
  auto modelRun = ROOT::RNTupleModel::CreateBare();
  modelRun->MakeField<std::uint64_t>("runId");

  static constexpr const char *Filename = "random.root";
  std::unique_ptr<TFile> file;
  if (rank == kRoot) {
    file.reset(TFile::Open(Filename, "RECREATE"));
  }
  ...
  RNTupleWriterMPI::Config configRun;
  configRun.fModel = std::move(modelRun);
  configRun.fNTupleName = "run";
  configRun.fStorage = Filename;
  configRun.fOptions.SetCompression(compression);
  if (mode & 0x10) {
    configRun.fOptions.SetUseDirectIO(true);
  }
  configRun.fOptions.SetMaxUnzippedPageSize(128 * 1024);
  configRun.fWriteAlignment = config.fWriteAlignment;
  configRun.fSendData = config.fSendData;
  configRun.fSendKey = config.fSendKey;
  configRun.fReduceRootContention = config.fReduceRootContention;
  ...
  // Create the writers and start filling the RNTuple.
  ...
  auto writerRun = RNTupleWriterMPI::Append(std::move(configRun), file.get(),
                                            kRoot, MPI_COMM_WORLD);
  auto entryRun = writerRun->CreateEntry();
  auto runId = entryRun->GetPtr<std::uint64_t>("runId");
  ...
  // Commit the datasets to make sure all data is written.
  writer->CommitDataset();
  writerRun->CommitDataset();
  ...
  if (rank == kRoot) {
    // Close the TFile, writing its structure and the key list.
    file.reset();
